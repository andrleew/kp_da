#ifndef BASE_INVERTED_INDEX_H
#define BASE_INVERTED_INDEX_H
...
class BaseInvertedIndexElement
{
public:
    BaseInvertedIndexElement();
    ~BaseInvertedIndexElement();
...
public:
    void Print();
    void Insert(const size_t, uint);
    void EncodeAll();
    void Write(std::ofstream&);
    void Read(std::ifstream&);
    DynamicBitSet Get( const size_t );

    inline size_t Size() { return _indexes.size();  } 

private:
    std::unordered_map<size_t, BaseInvertedIndexElement> _indexes;
};
